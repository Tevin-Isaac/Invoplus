...
function StepCard({ step, index }: { step: Step; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="rounded-[2rem] border-2 border-white/15 bg-slate-900 p-8 shadow-2xl"
    >
      <div className="flex items-start justify-between gap-6 mb-6">
        <span className="font-data text-5xl font-bold text-white/10">{step.number}</span>
...
export function HowItWorks() {
  ...
  return (
    <section id="how-it-works" className="relative bg-slate-950 border-t border-slate-800">
      {/* Ambient glow + finance-themed particles, `absolute inset-0` so this
          layer spans the section's real content-driven height and never
          adds its own layout space. */}
      ...
      <div className="relative max-w-6xl mx-auto px-6 lg:px-10 pb-16 lg:pb-20 grid gap-5 md:grid-cols-2">
        {steps.map((step, i) => (
